    ...
    >
      ...
      <div className="flex items-center justify-between border-b-2 border-black pb-3 mb-3">
        <div className="flex items-center space-x-2">
          <div className={`p-1.5 border-2 border-black rounded-lg shadow-[2px_2px_0px_#000] ${response.is_refusal ? 'bg-[#FF2A55] text-white' : 'bg-[#FFE500] text-black'}`}>
            <Sparkles className="w-4 h-4 fill-current" />
          </div>
          <span className="text-xs font-black uppercase text-slate-950 font-display tracking-wide">
            {response.is_refusal ? "🛡️ Safety Guardrail Intercepted" : "Grounded Answer"}
          </span>
        </div>

        <div className="flex items-center space-x-2">
          <span
            className={`sticker-badge text-[10px] font-black font-mono-data cursor-help ${response.is_refusal ? 'bg-[#FF2A55] text-white' : 'bg-[#00F5D4] text-black'}`}
          ...
          >
            {response.is_refusal ? "🛡️ Guardrail: 0.1 ms" : `⚡ RAG: ${((response.latency.total_retrieval_ms || 0) + (response.latency.guardrail_input_ms || 0) + (response.latency.guardrail_output_ms || 0)).toFixed(1)} ms`}
          </span>
          <button
            onClick={onDismiss}
      ...
      {/* Answer Paragraph */}
      <div className={`text-sm font-semibold leading-relaxed font-sans mb-4 ${response.is_refusal ? 'text-red-700 bg-red-50 p-3 rounded-xl border border-red-200' : 'text-slate-900'}`}>
        {response.answer}
      </div>

      {/* Trust & Citation Strip */}
      <div className="pt-3 border-t-2 border-black flex flex-wrap items-center justify-between gap-2 text-xs">
        <span className={`flex items-center font-bold text-[11px] ${response.is_refusal ? 'text-amber-800' : 'text-emerald-800'}`}>
          <CheckCircle2 className={`w-4 h-4 mr-1 ${response.is_refusal ? 'text-amber-600 fill-amber-100' : 'text-emerald-600 fill-emerald-100'}`} />
          {response.is_refusal ? "Enforced: Safety & Abstention Guardrail" : "Verified with MSMARCO-XI Knowledge Base"}
        </span>
